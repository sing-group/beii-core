...
origin=_ref2$.origin,destination=_ref2$.destination,_ref2$$orderedDestina=_ref2$.orderedDestinations,orderedDestinations=_ref2$$orderedDestina===undefined?[]:_ref2$$orderedDestina,_ref2$$destinationInd=_ref2$.destinationIndex,destinationIndex=_ref2$$destinationInd===undefined?1:_ref2$$destinationInd,type=_ref2[1],_ref2$2=_ref2[2],transportMode=_ref2$2===undefined?'driving':_ref2$2;var coords,routeResponse,route;return _regenerator2.default.async(function getRoute$(_context){while(1){switch(_context.prev=_context.next){case 0:

coords=origin.longitude+','+origin.latitude+';'+destination.longitude+','+destination.latitude;_context.next=3;return _regenerator2.default.awrap(

fetch('https://api.mapbox.com/directions/v5/mapbox/'+transportMode+'/'+coords+'?geometries=geojson&access_token='+_Config2.default.accessToken));case 3:routeResponse=_context.sent;_context.next=6;return _regenerator2.default.awrap(
...
console.log("An error occurred, re-routing to next local");_context.next=14;return _regenerator2.default.awrap(
fetch('https://api.mapbox.com/directions/v5/mapbox/'+transportMode+'/'+coords+'?geometries=geojson&access_token='+_Config2.default.accessToken));case 14:routeResponse=_context.sent;_context.next=17;return _regenerator2.default.awrap(
...
origin=_ref4.origin,destination=_ref4.destination,waypoints=_ref4.waypoints,type=_ref4.type,_ref4$transportMode=_ref4.transportMode,transportMode=_ref4$transportMode===undefined?'driving':_ref4$transportMode,_ref4$distance=_ref4.distance,distance=_ref4$distance===undefined?100:_ref4$distance;var points,coords,radiuses,uri,routeResponse,route;return _regenerator2.default.async(function getRouteWithWaypoints$(_context3){while(1){switch(_context3.prev=_context3.next){case 0:if(!(
...
uri='http://router.project-osrm.org/trip/v1/'+transportMode+'/'+coords+'?source=first&destination=last&radiuses='+radiuses+'&geometries=geojson&roundtrip=false';_context3.next=8;return _regenerator2.default.awrap(

fetch(uri));case 8:routeResponse=_context3.sent;_context3.next=11;return _regenerator2.default.awrap(
