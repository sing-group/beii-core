...
position=_ref2[0],destinations=_ref2[1],transportMode=_ref2[2];var queryCoords,response,distances,orderedDests;return _regenerator2.default.async(function orderByDuration$(_context){while(1){switch(_context.prev=_context.next){case 0:if(!(
...
queryCoords=destinations.map(function(d){return d.longitude+','+d.latitude;}).join(';');_context.next=6;return _regenerator2.default.awrap(

fetch('http://router.project-osrm.org/table/v1/'+transportMode+'/'+queryCoords+'?sources=0'));case 6:response=_context.sent;_context.next=9;return _regenerator2.default.awrap(
